...
class QuizRequest(BaseModel):
    ...
    # Not something PARSE_REQUEST_SYSTEM infers from free text below — this
    # is a user-level preference (see db.py's users.preferred_language) that
    # api.py sets on the request dict directly, same as it does for
    # class_id/lecture_id. Kept here purely so QuizRequest stays a complete
    # model of what generate_question_set() actually accepts.
    language: str = "en"
...
Write all question text, answer options, explanations, and answer summaries in {language}. Keep
numeric values, units (e.g. "ohm", "V", "A"), and variable-style names (e.g. R_TH, V_OC, I_SC)
exactly as they are — translate the surrounding natural-language text only, never the formulas,
units, or variable names themselves.
"""


def build_language_instruction(language):
    """Returns the language-localization block for the prompt, or "" for
    English — leaving the prompt byte-for-byte what it already was for the
    (still-default) English case, rather than adding a no-op instruction
    that could itself nudge output quality."""
    language_name = resolve_language_name(language)
    if not language or language_name.strip().lower() == "english":
        return ""
    return LANGUAGE_INSTRUCTION_TEMPLATE.format(language=language_name)
...
def build_prompt(retrieved_results, request, topic, needs_diagram=False):
    ...
    language_instruction = build_language_instruction(request.get("language", "en"))
...
def attach_generated_diagrams(questions, output_dir="generated_diagrams"):
    ...
    output_dir defaults to a bare relative path for standalone/CLI use
    (matches render_circuit_diagram.py/render_chart_diagram.py's own
    defaults) — api.py's /quiz endpoint passes an absolute, stable location
    instead (see generate_question_set's diagram_output_dir param), so
    every quiz-generated image lands in one predictable place regardless of
    the API process's current working directory at request time, and can
    be served back to the frontend afterward.

    image_file is stored as an ABSOLUTE path regardless of what output_dir
    was — api.py needs this to reliably compute a relative sub-path (for
    building a servable URL) against its own known diagrams root, the same
    reasoning as screen_to_text.py/pptx_to_text.py's image_file paths."""
    diagram_count = 0
    for q in questions:
        ...
        try:
            if kind == "circuit" and spec.get("components"):
                ...
                q["diagram"] = {"caption": circuit_caption(spec), "image_file": os.path.abspath(path)}
                diagram_count += 1
            elif kind == "chart":
                path = unique_chart_path(output_dir=output_dir)
                render_chart(spec, path)
                q["diagram"] = {"caption": chart_caption(spec), "image_file": os.path.abspath(path)}
                diagram_count += 1
            else:
...
def generate_question_set(request, persist_directory=PERSIST_DIR, diagram_output_dir="generated_diagrams"):
    ...
    if needs_diagram:
        diagram_count = attach_generated_diagrams(questions, output_dir=diagram_output_dir)
        if diagram_count == 0:
